...
class StreamURLFetcherInternal(YouTube):
    ...
    def __init__(self):
        ...
        if not isPyTubeInstalled:
            raise Exception(
                "ERROR: PyTube is not installed. To use this functionality of youtube-search-python, PyTube must be installed."
            )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    "https://www.youtube.com/watch", timeout=None
                )
        ...
        if not videoFormats["streamingData"]:
            try:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    self._player_response["player_response"] = await loads(
                        parse_qs(response.text)["player_response"][0]
                    )
            except:
                raise Exception("ERROR: Could not make request.")
