        ...
        for element in self.responseSource:
            if playerResponseKey in element.keys():
                if "videoDetails" in element[playerResponseKey].keys():
                    ...
                    Valid video ID.
                    """
                    self.videoComponent = await self.__getVideoComponent(
                        element[playerResponseKey], mode
                    )
                    break
                else:
                    """
    ...
    async def __getVideoComponent(self, element: dict, mode: str) -> dict:
        ...
        if mode in ["getInfo", None]:
            ...
            component = {
                ...
                "description": await self.__getValue(
                ...
                "keywords": await self.__getValue(
                    element, ["videoDetails", "keywords"]
                ),
                "publishDate": await self.__getValue(
...
class PlaylistInternal:
        ...
        for response in self.responseSource:
            if "response" in response.keys():
                ...
                if not playlistElement["info"]:
                    raise Exception("ERROR: Could not parse YouTube response.")
                self.playlistComponent = await self.__getPlaylistComponent(
                    playlistElement, self.componentMode
                )
    ...
    async def __getNextComponents(self) -> None:
        ...
        continuationElements = await self.__getValue(
            self.responseSource,
            [
        ...
        for videoElement in continuationElements:
            if playlistVideoKey in videoElement.keys():
                videoComponent = {
                    "id": await self.__getValue(
                        videoElement, [playlistVideoKey, "videoId"]
                    ),
                    "title": await self.__getValue(
                    ...
                    "duration": await self.__getValue(
            ...
            if continuationItemKey in videoElement.keys():
                self.continuationKey = await self.__getValue(
            ...
            for infoElement in element["info"]:
                if playlistPrimaryInfoKey in infoElement.keys():
                    component = {
                        ...
                        "videoCount": await self.__getValue(
                            infoElement,
                            [playlistPrimaryInfoKey, "stats", 0, "runs", 0, "text"],
                        ),
                        "viewCount": await self.__getValue(
                            infoElement,
...
class SuggestionsInternal:
    ...
    async def get(
        self, query: str, language: str = "en", region: str = "US", timeout: int = None
    ) -> dict:
        self.query = query
    ...
    async def __parseSource(self) -> None:
        try:
            self.responseSource = await loads(
